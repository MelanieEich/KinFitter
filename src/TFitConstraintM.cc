  ...
  ,_ParList2(0)
{
  ...
  if (Mass >= 0) {
    ...
    std::cout<<"NegativeMassConstr" << "Mass constraint in TFitConstraintM cannot be set to a negative value, will be set to 0."<<std::endl;
    _TheMassConstraint = 0.;
  }
}
  ...
  ,_ParList2(0) 
{
  ...
  if (Mass >= 0) {
    ...
    std::cout << "Mass constraint in TFitConstraintM cannot be set to a negative value, will be set to 0."<<std::endl;
    _TheMassConstraint = 0.;
  }
}
...
TMatrixD* TFitConstraintM::getDerivative( TAbsFitParticle* particle ) {
  ...
  if( OnList( &_ParList1, particle) ) {
    ...
    if( Pf.M() == 0. ) {
      std::cout<<"KinFitter"<< "Division by zero in "<< IsA()->GetName() << " (named " << GetName() << ", titled " << GetTitle()<< ") will lead to Inf in derivative matrix for particle " << particle->GetName() << "."<<std::endl;
    }
    //    std::cout<<"Pf.M "<<Pf.M()<<std::endl;
    Factor = 1./ Pf.M();
