...
void TFitConstraintMGaus::setMassConstraint(Double_t Mass, Double_t Width) { 
  ...
  _width = Width;
  setCovMatrix( 0 );
  if(!Mass) //throw std::exception("Configuration");
...
Double_t TFitConstraintMGaus::getCurrentValue() {
  ...
  // std::cout<<"CalcMass List 1 "<<CalcMass(&_ParList1,false)<<std::endl;
  // std::cout<<"CalcMass List 2 "<<CalcMass(&_ParList2,false)<<std::endl;
  // std::cout<<"Mass constraint "<<_TheMassConstraint<<std::endl;

  // std::cout<<"alpha "<<_parameters(0,0)<<std::endl;
  return CurrentValue;
