...
void TAbsFitConstraint::setCovMatrix(const TMatrixD* theCovMatrix) {
  ...
  std::cout<<"WrongMatrixSize "<< GetName()<<"::setCovMatrix - Measured alpha covariance matrix needs to be a "  << _nPar << "x" << _nPar <<" matrix."<<std::endl;
  }
...
void TAbsFitConstraint::setCovMatrixFit(const TMatrixD* theCovMatrixFit) {
  ...
  std::cout<<"WrongMatrixSize" << GetName() << "::setCovMatrixFit - Fitted covariance matrix needs to be a " << _nPar << "x" << _nPar << " matrix."<<std::endl;
  }
...
void TAbsFitConstraint::calcCovMatrixDeltaAlpha() {
  ...
  if(_covMatrixFit.GetNrows() == _nPar && _covMatrixFit.GetNcols() == _nPar)
    ...
    std::cout<<"WrongMatrixSize"<< GetName() << "::calcCovMatrixDeltaAlpha - _covMatrixFit probably not set."<<std::endl;
}
...
void TAbsFitConstraint::setParIni(const TMatrixD* parini) {
  ...
  if (parini == 0) return;
    ...
    std::cout<<"WrongMatrixSize" << GetName() << "::setParIni - Matrices don't fit."<<std::endl;
    return;
  }
