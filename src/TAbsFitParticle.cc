...
void TAbsFitParticle::setCovMatrix(const TMatrixD* theCovMatrix) {
  ...
  if(theCovMatrix==0) {
    ...
    std::cout<<"WrongMatrixSize" << GetName() << "::setCovMatrix - Covariance matrix needs to be a " << _nPar << "x" << _nPar << " matrix."<<std::endl;
  }
...
void TAbsFitParticle::calcCovMatrixDeltaY() {
  ...
  if(_covMatrixFit.GetNrows() == _nPar && _covMatrixFit.GetNcols() == _nPar)
    ...
    std::cout<<"WrongMatrixSize" << GetName() << "::calcCovMatrixDeltaY - _covMatrixFit probably not set."<<std::endl;
}
