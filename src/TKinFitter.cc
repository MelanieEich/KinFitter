...
Int_t TKinFitter::fit() {
  ...
  // print status
    if( _verbosity == 1 ) {
       std::cout<<"Current chi2: "<<getS()<<std::endl;
       // double thadmass = (*get4Vec(0)+ *get4Vec(1)+ *get4Vec(2)).M();
       // double tlepmass = (*get4Vec(3)+ *get4Vec(4)+ *get4Vec(5)).M();
       // std::cout<<"top had mass "<<thadmass<<std::endl;
       // std::cout<<"top lep mass "<<tlepmass<<std::endl;
       // std::cout<<"Matrix _A: "<<std::endl;
       // _A.Print();
       std::cout<<"Matrix _B: "<<std::endl;
       _B.Print();
       std::cout<<"Matrix _VBinv: "<<std::endl;
       _VBinv.Print();
       std::cout<<"Matrix _VB: "<<std::endl;
       _VB.Print();
       std::cout<<"Matrix _V: "<<std::endl;
       _V.Print();
       std::cout<<"Matrix _deltaY: "<<std::endl;
       _deltaY.Print();
       // std::cout<<"Matrix _deltaA: "<<std::endl;
       // _deltaA.Print();
       // std::cout<<"Matrix _C32T: "<<std::endl;
       // _C32T.Print();
       std::cout<<"Matrix _c: "<<std::endl;
       _c.Print();
     }
  ...
  do {
    ...
    // std::cout<<"deltaF "<<currF-prevF<<std::endl;
    ...
    Int_t nstep = 0;
    while (currF >= prevF) {
      // std::cout<<"enter reducing step"<<std::endl;
      // std::cout<<"currF "<<currF<<std::endl;
     nstep++;
      if (nstep == 10) break;
  ...
  //  std::cout<<"isConverged "<<isConverged<<std::endl;
  // std::cout<<"nbIter "<<_nbIter<<std::endl;
  // std::cout<<"_status "<<_status<<std::endl;
  // Calculate covariance matrices
  //  std::cout<<"calcB: "<<std::endl;
  calcB();
  //std::cout<<"calcVB: "<<std::endl;
  calcVB();
  ...
  //std::cout<<"calcC11, 31, 33, calcVFit, applyVFit: "<<std::endl;
...
Bool_t TKinFitter::calcV() {
  ...
  try {
    ...
    throw std::runtime_error("Failed to invert covariance matrix V. Fit will be aborted.");
    _status = -10;
  }
...
Bool_t TKinFitter::calcB() {
  ...
  for (unsigned int indexConstr = 0; indexConstr < _constraints.size(); indexConstr++) {
    ...
    for (unsigned int indexParticle = 0; indexParticle < _measParticles.size(); indexParticle++) {
      ...
      TMatrixD* derivParticle = particle->getDerivative();
      TMatrixD* derivConstr = _constraints[indexConstr]->getDerivative( particle );
      //in the step ahead the Number of free Parameter outline is somewhere hidden
      TMatrixD deriv( *derivConstr,  TMatrixD::kMult, *derivParticle );
      // if (_verbosity >= 3) {
...
Bool_t TKinFitter::calcVB() {
  ...
  _VB = _VBinv;
  try {
    // std::cout<<"vb.invert "<<std::endl;
    // _VB.Print();
    _VB.Invert();
    // _VB.Print();
    // std::cout<<"vb.inverted "<<std::endl;
  } catch (std::exception& e) {
    throw std::runtime_error("Failed to invert matrix VB. Fit will be aborted.");
    _status = -10;
  }
...
Bool_t TKinFitter::calcVA() {
  ...
  try {
    ...
    throw std::runtime_error("Failed to invert matrix VA. Fit will be aborted.");
    _status = -10;
  }
